...
export default function ParticlesBg() {
  ...
  useEffect(() => {
    ...
    function createParticle(x: number, y: number, fromMouse = false) {
      ...
      // More varied sizes
      const size = fromMouse ? Math.random() * 12 + 3 : Math.random() * 5 + 1
      ...
      // More varied speeds
      const speedFactor = fromMouse ? 3 : 1
      ...
      // Add glow effect
      const glow = fromMouse ? 10 : 5

      // Add life and fade properties
      const life = fromMouse ? 100 : 200
      ...
      return {
        ...
        fade,
        opacity: 1,
        // Add connection lines only between mouse particles
        fromMouse,
      }
    }
    ...
    const animate = () => {
      ...
      ctx.fillStyle = isDark ? "rgba(10, 10, 20, 0.1)" : "rgba(255, 255, 255, 0.1)"
      ...
      if (Math.random() < 0.05 && particles.length < 200) {
        particles.push(createParticle(0, 0))
      }
  ...
  return (
    <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full pointer-events-none" style={{ zIndex: -10 }} />
  )
}
